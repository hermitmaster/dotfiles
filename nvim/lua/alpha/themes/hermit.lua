...
local default_header = {
    ...
    val = {
      [[       __                        _ __        _         ]],
      [[      / /_  ___  _________ ___  (_) /__   __(_)___ ___ ]],
      [[     / __ \/ _ \/ ___/ __ `__ \/ / __/ | / / / __ `__ \]],
      [[    / / / /  __/ /  / / / / / / / /_ | |/ / / / / / / /]],
      [[   /_/ /_/\___/_/  /_/ /_/ /_/_/\__/ |___/_/_/ /_/ /_/ ]]
    },
    opts = {
        hl = "Title",
        shrink_margin = false,
...
local function button(sc, txt, keybind, keybind_opts)
    ...
    local opts = {
        ...
        -- width = 50,
        align_shortcut = "left",
        hl_shortcut = { { "Delimiter", 0, 1 }, { "Number", 1, #sc + 1 }, { "Delimiter", #sc + 1, #sc + 2 } },
        shrink_margin = false,
    }
    if keybind then
...
local function file_button(fn, sc, short_fn)
    ...
    if fn_start ~= nil then
        table.insert(fb_hl, { "Directory", #ico_txt - 2, #fn_start + #ico_txt - 2 })
    end
...
local section = {
    ...
    mru = {
        ...
        val = {
            { type = "padding", val = 1 },
            { type = "text", val = "MRU", opts = { hl = "Type" } },
            { type = "padding", val = 1 },
            {
                type = "group",
                val = function()
    ...
    mru_cwd = {
        ...
        val = {
            { type = "padding", val = 1 },
            { type = "text", val = mru_title, opts = { hl = "Type", shrink_margin = false } },
            { type = "padding", val = 1 },
            {
                type = "group",
                val = function()
