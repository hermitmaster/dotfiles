if (( ! $+commands[git] )); then
  echo "git is not installed. Please install it to use this plugin."
  return
fi
...
# Completions
################
if [[ -n $ZSH_VERSION ]] && type _git &>/dev/null; then
  # Enable completion for all git aliases and functions defined above
  for alias in ${(k)aliases}; do
    if [[ $aliases[$alias] == git* ]]; then
      compdef _git $alias
    fi
  done
  # Also add completions for custom functions if they start with git_
