function ga -d "git add selector"
    ...
    set preview "
        ...
        if test (git status -s -- \$file | grep '^??') # diff with /dev/null for untracked files
            git diff --no-index -- /dev/null \$file | $GIT_PAGER | sed '2 s/added:/untracked:/'
        else
            git diff -- \$file | $GIT_PAGER
        end
        "
    set opts "
    ...
    set files (git -c color.status=always -c status.relativePaths=true status -su |
        fgrep -e "$changed" -e "$unmerged" -e "$untracked" |
